    ...
    (roomInfo, checkIn, checkOut) => {
      ...
      applicableEvents.forEach((event) => {
        ...
        if (
          ...
          selectedEventStartDate = event.startDate;
          selectedEventEndDate = event.endDate;
          totalFixedDiscount = maxFixedDiscount;
        }
      });
        ...
        (room) => {
          ...
          const applicableHotelCoupons = (hotelCoupons || []).filter(
            (coupon) => {
              ...
              const isDateValid =
                coupon.startDate <= todayStr && coupon.endDate >= todayStr;
          ...
          const applicableCustomerCoupons = (customerCoupons || []).filter(
            (coupon) => {
              ...
              const isDateValid =
                coupon.endDate >= todayStr && coupon.startDate <= todayStr;

              console.log(
                '[RoomSelection] Customer coupon filter conditions:',
                {
                  coupon: {
                    code: coupon.code,
          ...
          const uniqueCoupons = Array.from(
            new Map(
              applicableCoupons.map((coupon) => [coupon.couponUuid, coupon])
            ).values()
          );
            ...
            uniqueCoupons
              .map((coupon) => {
                ...
                if (coupon.discountType === 'percentage') {
                  discountValue =
                    totalPriceForCalc * (validatedDiscountValue / 100);
          ...
          if (bestCoupon && !(discount > 0 || fixedDiscount > 0)) {
            if (bestCoupon.discountType === 'percentage') {
              ...
              couponFixedDiscount = bestCoupon.validatedDiscountValue;
  ...
  const handleApplyCoupon = (roomInfo, coupon) => {
    ...
    if (coupon) {
      if (coupon.discountType === 'percentage') {
        ...
        couponFixedDiscount = Number(coupon.discountValue) || 0;
        couponDiscountType = 'fixed';
    ...
    return availableRooms.map((room, index) => {
      ...
      if (selectedCoupon) {
        if (room.discount > 0 || room.fixedDiscount > 0) {
          ...
          couponFixedDiscount = Number(selectedCoupon.discountValue) || 0;
          couponDiscount = 0;
          couponDiscountType = 'fixed';
          couponDiscountValue = couponFixedDiscount;
                ...
                return (
                  <Box key={room.uniqueKey} w="100%">
                    <Flex
                      ...
                      px={{ base: 3, sm: 4 }}
                      py={4}
                      bg={room.isSoldOut ? 'gray.100' : 'white'}
                      opacity={room.isSoldOut ? 0.6 : 1}
                        ...
                        {photoCount > 1 && (
                          ...
                          >
                            {photos.map((photo, idx) => (
                              <Box
                                key={photo.photoUrl} // photoUrl을 key로 사용
                                w="8px"
                                h="8px"
                                borderRadius="full"
                                bg={
                        ...
                        <Flex justify="space-between" w="100%" align="center">
                          ...
                          <Text fontSize="xs" color="red.500">
                            {(room.availableRooms || room.stock || 0) === 0
                              ? '' // "객실 마감" 텍스트 제거
                              : `남은 객실 ${
                                  room.availableRooms || room.stock || 0
                                }개`}
                          </Text>
                        </Flex>
                        <Flex justify="space-between" w="100%" align="center">
                          <Flex display="inline-flex" align="center" gap={1}>
                            {displayedAmenities.map((amenity) => (
                              <Box key={amenity.nameKor} color="gray.500">
                                {' '}
                                {/* nameKor을 key로 사용 */}
